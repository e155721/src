...
for (i in 1:5) {
  ...
  # matchingrate path
  ansrate.file <- paste("../../Alignment/", ansrate, "_pf-pmi_p", i, "_", format(Sys.Date()), ".txt", sep = "")
  
  # result path
  output.dir <- paste("../../Alignment/", multiple, "_pf-pmi_p", i, "_", format(Sys.Date()), "/", sep = "")
  if (!dir.exists(output.dir))
    dir.create(output.dir)
  
  # Compute the scoring matrix using the PMI method.
  input.list <- MakeInputList(files)
  load(paste("../../Alignment/12-11_ex-02/scoring_matrix_p", i, ".RData", sep=""))
