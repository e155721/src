...
E <- 1
denom.vec <- c(1, 10, 100, 100, 1000, 10000, 100000)
for (denom in denom.vec) {
  ...
  # matchingrate path
  ansrate.file <- paste("../../Alignment/ansrate_pmi_", format(Sys.Date()), "_02_", E, ".txt", sep = "")
  
  # result path
  output.dir <- paste("../../Alignment/pairwise_pmi_", format(Sys.Date()), "_02_", E, "/", sep = "")
    ...
    while (1) {
      ...
      for (i in 1:V) {
        for (j in 1:V) {
          ...
          if (a!=b) {
            p.xy <- (co.mat[a, b]/N)+E
            p.x <- (g(a, newcorpus)/N)+E
            p.y <- (g(b, newcorpus)/N)+E
